...
contract NominationDAO is AccessControl {
  ...
  function grant_member(address newMember) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    if (staking.is_nominator(address(this))) {
      ...
      if (address(this).balance < MinNominatorStk) {
        ...
        staking.nominate(
          ...
          staking.collator_nomination_count(target),
          staking.nominator_nomination_count(address(this))
        );
      }
    }
  }
  ...
  function cash_out(address payable account) public onlyRole(MEMBER) {
    if ((totalStake - memberStakes[msg.sender]) < MinNominatorStk) {
      ...
      uint256 amount = memberStakes[msg.sender];
  ...
  function retrieve_revoked_cash(address payable account)
    public
    onlyRole(MEMBER)
  {
    uint256 stakingRewards = (address(this).balance - memberStakes[msg.sender])
      .mul(memberStakes[msg.sender])
      .div(totalStake);
    require(
      address(this).balance >= memberStakes[msg.sender] + stakingRewards,
